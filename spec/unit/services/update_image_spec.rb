describe JobBoard::Services::UpdateImage do
  ...
  it 'updates an image' do
    ...
    expect(JobBoard::Models::Image).to receive(:[]).and_return(image0)
  ...
  context 'when the image does not exist' do
    it 'is a no-op' do
      expect(JobBoard::Models::Image).to receive(:[]).and_return(nil)
      expect(described_class.run(params: { 'id' => '1' })).to eql(nil)
    end
  end
end
